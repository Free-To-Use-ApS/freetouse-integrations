...
- **Audio playback:** Use the Web Audio API with a `GainNode` between the audio element and speakers. Fade out (60ms `linearRampToValueAtTime` to 0) before switching tracks, fade in (60ms ramp) on play. This eliminates the audible pop/blip when switching. The fade-in target is the per-track loudness gain `waveformToGain(track.waveform)` (from `@freetouse/api`), not a hardcoded `1`, so loud tracks settle quieter — see the playbook's "Loudness normalization" section.
